...
function Button({ name, folderId, src, onClick, className, ...props }: ButtonProps) {
  ...
  return (
    <>
      <button
        className={cn(
          'relative text-p1 flex items-center justify-between py-1.5 pl-10 pr-4 w-full bg-white border border-inputB/50 rounded-sm hover:bg-p1/10 dark:hover:bg-p1/50 dark:hover:text-foreground/50 transition-colors duration-300',
          className
        )}
        onClick={() => {router.push(pathname + "/"+ folderId)}}
        {...props}
      >
        <div className="icon absolute -top-[1px] -left-1 bg-p1 pl-4 pr-6 py-1.5 rounded">
